...
vector<vector<double> > Im2Col::im2col(const vector<vector<vector<vector<double> > > > images,
    ...
    const int n_channels = images[0].size();
    const int img_height = images[0][0].size();
    const int img_width = images[0][0][0].size();


    const int new_width = int(img_width - filter_width + 2 * padding_width) + 1;
    const int new_height = int(img_height - filter_height + 2 * padding_height) + 1;

    const int col_height = filter_height * filter_width * n_channels;
    const int col_width = batch_size * new_height * new_width;
...
vector<vector<vector<vector<double> > > > Im2Col::pad(const vector<vector<vector<vector<double> > > > images,
                                                      const int padding_height, const int padding_width) {
    vector<vector<vector<vector<double> > > > copied_images = images;
    const int batch_size = copied_images.size();
    const int n_channels = copied_images[0].size();
    const int img_height = copied_images[0][0].size();
    const int img_width = copied_images[0][0][0].size();
...
int main() {
    ...
    vector<vector<vector<vector<double> > > >
            images(10, vector<vector<vector<double> > >(10, vector<vector<double> >(256, vector<double>(256, 1.0))));
